...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, num_layers):
            ...
            # Affine
            if self.normalization in ["batchnorm", "layernorm"] and i != num_layers:
                out, fc_cache = affine_forward(out, W, np.zeros_like(W.shape[1]))
            else:
                out, fc_cache = affine_forward(out, W, b)

            # Batch norm/layer norm
            if self.normalization == "batchnorm" and i != num_layers:
                gamma = self.params[f'gamma{i}']
                beta = self.params[f'beta{i}']
                out, bn_cache = batchnorm_forward(out, gamma, beta, self.bn_params[i - 1])
            elif self.normalization == "layernorm" and i != num_layers:
                gamma = self.params[f'gamma{i}']
                beta = self.params[f'beta{i}']
        ...
        # Final affine layer (no ReLU)
        W_final = self.params[f'W{num_layers}']
        b_final = self.params[f'b{num_layers}']
        ...
        grads[f'W{num_layers}'] = dW + reg * self.params[f'W{num_layers}']
        ...
        for i in reversed(range(1, num_layers)):
            ...
            grads[f'W{i}'] = dW + reg * self.params[f'W{i}']
            grads[f'b{i}'] = db
