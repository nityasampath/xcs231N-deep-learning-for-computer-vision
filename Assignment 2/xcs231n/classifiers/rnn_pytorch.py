...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # Step 1: Compute initial hidden state from image features
        h0 = features.mm(W_proj) + b_proj  # (N, H)

        # Step 2: Embed input words
        x_embed = W_embed[captions_in]  # (N, T, W)

        # Step 3: Forward pass through RNN
        if self.cell_type == "rnn":
            h = rnn_forward(x_embed, h0, Wx, Wh, b)  # (N, T, H)

        # Step 4: Compute vocabulary scores
        N, T, H = h.shape
        V = W_vocab.shape[1]
        scores = h.reshape(N * T, H).mm(W_vocab) + b_vocab  # (N*T, V)

        # Step 5: High-precision temporal softmax
        captions_out_flat = captions_out.reshape(N * T)
        mask_flat = mask.reshape(N * T)

        # Numerically stable log softmax
        max_scores = scores.max(dim=1, keepdim=True)[0]
        log_probs = scores - max_scores - torch.log(torch.exp(scores - max_scores).sum(dim=1, keepdim=True))
        # Pick log probability of correct class
        loss_all = -log_probs[torch.arange(N * T), captions_out_flat]
        # Mask out <NULL> tokens and normalize
    ...
    def sample(self, features, max_length=30):
        ...
        prev_h = features.mm(W_proj) + b_proj  # (N, H)

        # Step 2: Initialize first word to <START> token
        prev_word = self._start * torch.ones(N, dtype=torch.long, device=features.device)

        for t in range(max_length):
            # (1) Embed the previous word
            x_embed = W_embed[prev_word]  # (N, W)

            # (2) Make an RNN step
            next_h = rnn_step_forward(x_embed, prev_h, Wx, Wh, b)  # (N, H)

            # (3) Compute scores over the vocabulary
            scores = next_h.mm(W_vocab) + b_vocab  # (N, V)

            # (4) Select the word with highest score
            next_word = scores.argmax(dim=1)  # (N,)

            # Write the word to captions
            captions[:, t] = next_word

            # Update hidden state and previous word
            prev_h = next_h
            prev_word = next_word
