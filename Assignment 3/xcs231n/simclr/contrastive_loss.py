...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        # ### START CODE HERE ###
        # Compute numerator for both directions
        num_k = torch.exp(sim(z_k, z_k_N) / tau)
        num_N = torch.exp(sim(z_k_N, z_k) / tau)

        # Denominator sums over all indices except itself
        denom_k = 0
        denom_N = 0

        for m in range(2 * N):
            if m != k:
                denom_k += torch.exp(sim(z_k, out[m]) / tau)
            if m != (k + N):
                denom_N += torch.exp(sim(z_k_N, out[m]) / tau)

        # Loss terms
        loss_k = -torch.log(num_k / denom_k)
        loss_N = -torch.log(num_N / denom_N)

        total_loss += loss_k + loss_N
        # ### END CODE HERE ###
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(out_left * out_right, dim=1, keepdim=True) / (norms_left * norms_right)
    # ### END CODE HERE ###
...
def compute_sim_matrix(out):
    ...
    # ### START CODE HERE ###
    norms = torch.linalg.norm(out, dim=1, keepdim=True)      # [2N,1]
    out_normed = out / norms                                 # [2N,D]
    sim_matrix = out_normed @ out_normed.T                    # [2N,2N]
    # ### END CODE HERE ###
...
def simclr_loss_vectorized(out_left, out_right, tau, device="cuda"):
    ...
    # ### START CODE HERE ###
    pos = torch.cat([
        sim_matrix[torch.arange(N), torch.arange(N, 2*N)],
        sim_matrix[torch.arange(N, 2*N), torch.arange(N)],
    ], dim=0)  # [2N]
    # ### END CODE HERE ###
    ...
    numerator = None
    # ### START CODE HERE ###
    numerator = torch.exp(pos / tau).view(2 * N, 1)
    # ### END CODE HERE ###
