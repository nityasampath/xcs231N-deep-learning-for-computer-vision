...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        memory = memory.unsqueeze(1)                   # (N, 1, W) to act as "sequence length 1"

        tgt_mask = torch.tril(torch.ones(T, T, device=x.device)).bool()  # (T, T)

        out = self.transformer(x, memory, tgt_mask=tgt_mask)  # (N, T, W)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Average pool over patches to get image-level feature
