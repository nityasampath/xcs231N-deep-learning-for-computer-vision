...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, 0::2] = torch.sin(position * div_term)              # sine for even indices
        pe[0, :, 1::2] = torch.cos(position * div_term)              # cosine for odd indices
        # ### END CODE HERE ###
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = Q.view(N, S, H, self.head_dim).transpose(1, 2)  # (N, H, S, d_k)
        K = K.view(N, T, H, self.head_dim).transpose(1, 2)  # (N, H, T, d_k)
        V = V.view(N, T, H, self.head_dim).transpose(1, 2)  # (N, H, T, d_k)
        ...
        attn_weights = self.attn_drop(attn_weights)  # optional dropout

        context = torch.matmul(attn_weights, V)  # (N, H, S, d_k)
        context = context.transpose(1, 2).contiguous().view(N, S, self.emd_dim)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # ### START CODE HERE ###
        # Step 1: reshape into (N, C, num_patches_h, patch_size, num_patches_w, patch_size)
        ...
        # Step 2: permute axes to (N, num_patches_h, num_patches_w, C, patch_size, patch_size)
        x = x.permute(0, 2, 4, 1, 3, 5)

        # Step 3: flatten each patch to a vector (patch_size*patch_size*C)
