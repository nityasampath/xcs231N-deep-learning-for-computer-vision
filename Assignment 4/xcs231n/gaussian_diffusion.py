...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        # ### START CODE HERE ###
        sqrt_alpha_cumprod_t = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        sqrt_one_minus_alpha_cumprod_t = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)

        x_start = (x_t - sqrt_one_minus_alpha_cumprod_t * noise) / sqrt_alpha_cumprod_t
        # ### END CODE HERE ###
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        pred_noise = (x_t - sqrt_alpha_cumprod_t * x_start) / sqrt_one_minus_alpha_cumprod_t
        # ### END CODE HERE ###
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        if self.objective == "pred_noise":
            model_out = self.predict_start_from_noise(x_t, t, model_out)

        # Get the posterior mean and standard deviation, sample backwards
        mean, std = self.q_posterior(torch.clamp(model_out, -1, 1), x_t, t)
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        loss = (loss_weight * (pred - target)**2).mean()
        # ### END CODE HERE ###
        ####################################################################
